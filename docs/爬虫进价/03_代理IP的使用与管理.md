...
| 风控检测触发 | 412 | 数小时~1天 | 需更换IP |
| 严重违规 | 403 | 数天~永久 | 需更换IP |

### B站专用代理检测器

针对 B站，我们需要专门的代理检测器来确保代理能正常访问 B站服务：
...
async def main():
    ...
    try:
        ...
        # 搜索视频
        result = await crawler.search_videos("Python教程", page=1)
        if result and result.get("code") == 0:
            videos = result.get("data", {}).get("result", [])
            for video in videos[:5]:
                print(f"标题: {video.get('title')}")
                print(f"播放: {video.get('play')}")
...
1. **代理类型**：B站对代理检测严格，推荐使用高匿住宅代理
2. **请求频率**：单IP建议 0.5-1 秒/请求，避免触发频率限制
3. **完整请求头**：必须携带 User-Agent、Referer、Origin 等头信息
4. **Cookie 携带**：部分 API 需要登录态，代理请求也要携带 Cookie
5. **失败处理**：遇到 412/429 立即切换代理，避免 IP 被永久封禁
...
本章代理管理技术在第11章 B站综合实战项目中有重要应用：

| 本章内容 | 第11章对应实现 | 文件位置 |
|---------|--------------|---------|
| 代理池接口设计 | `ProxyPool` 实际应用 | `crawler/spider.py` |
| B站专用检测器 | 集成到爬虫客户端 | `client/bilibili_client.py` |
| 风控处理 | 响应码检测与重试 | `client/base_client.py` |
| 代理轮换策略 | 自动切换逻辑 | `crawler/spider.py` |
...
1. 先掌握本章代理池的设计思想和实现原理
2. 然后在第11章实战项目中查看代理的实际集成方式
3. 根据自己的代理资源情况，选择合适的代理获取器实现
