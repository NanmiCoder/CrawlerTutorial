...
> **实战目标**：本章以 **B站（bilibili.com）** 爬虫项目为例，讲解工程化开发规范。这些规范将直接应用于第11章的综合实战项目。
...
graph LR
    ...
    subgraph B站爬虫场景
        ...
        d3[请求超时重试] -.-> WARNING
        d4[单页解析失败] -.-> ERROR
        d5[登录失效/IP封禁] -.-> CRITICAL
    end
```

| 级别 | 使用场景 | B站爬虫示例 |
|------|----------|-------------|
| DEBUG | 详细的调试信息，如请求参数、响应内容片段 | WBI签名参数、API响应JSON |
| INFO | 正常的运行信息，如"开始爬取第N页"、"成功保存N条数据" | 搜索视频第N页、保存N个视频信息 |
| WARNING | 可恢复的异常，如"请求超时，准备重试" | B站API返回-412风控、请求超时重试 |
| ERROR | 严重错误但程序可继续，如"单个页面解析失败" | 单个视频详情获取失败 |
| CRITICAL | 致命错误，程序无法继续，如"数据库连接失败" | Cookie过期、IP被封禁 |
...
以下是 B站视频搜索 API 的日志使用示例：

```python
# bilibili_client.py - B站API客户端日志示例
from loguru import logger
import httpx

async def search_bilibili_video(client: httpx.AsyncClient, keyword: str, page: int = 1):
    """
    搜索B站视频
    ...
    logger.debug(f"[BilibiliClient] 搜索参数: keyword={keyword}, page={page}")
    ...
    try:
        ...
        if code == 0:
            ...
            logger.info(f"[BilibiliClient] 搜索成功: '{keyword}' 第{page}页, 获取{result_count}条结果")
            return data
        elif code == -412:
            logger.warning(f"[BilibiliClient] 触发风控: '{keyword}', code=-412")
            raise Exception("触发B站风控")
        elif code == -101:
            logger.critical(f"[BilibiliClient] Cookie已过期，需要重新登录")
            raise Exception("登录状态失效")
        else:
            logger.error(f"[BilibiliClient] API错误: code={code}, message={data.get('message')}")
            return None

    except httpx.TimeoutException:
        logger.warning(f"[BilibiliClient] 请求超时: {url}")
        raise
    except Exception as e:
        logger.exception(f"[BilibiliClient] 未知错误: {e}")
        raise
```

> **日志前缀约定**：在 B站爬虫项目中，我们使用 `[模块名]` 作为日志前缀，便于在日志文件中快速定位问题来源，如 `[BilibiliClient]`、`[BilibiliLogin]`、`[BilibiliCrawler]`。
...
以下是 B站爬虫项目的完整配置示例，与第11章综合实战项目保持一致：

```python
# config/settings.py - B站爬虫配置
from pydantic_settings import BaseSettings
from pydantic import Field
...
```python
# config/bilibili_config.py - B站API配置
"""B站 API 配置常量"""
...
```python
# exceptions/bilibili_exceptions.py
"""B站爬虫专用异常类"""
...
class BilibiliAPIError(BilibiliException):
    """B站API返回错误"""

    def __init__(self, code: int, message: str):
        self.code = code
        self.message = message
        super().__init__(f"B站API错误: code={code}, message={message}")


class BilibiliRiskControlError(BilibiliException):
    """B站风控异常（code=-412）"""
    pass


class BilibiliLoginExpiredError(BilibiliException):
    """B站登录状态过期（code=-101）"""
    pass


class BilibiliWBISignError(BilibiliException):
    """WBI签名错误"""
    pass


class BilibiliVideoNotFoundError(BilibiliException):
    """视频不存在"""
    pass
...
以下是第11章综合实战项目的完整目录结构：

```
11_进阶综合实战项目/
├── config/                 # 配置模块
│   ├── __init__.py
│   ├── settings.py         # 通用配置（爬虫类型、登录方式等）
│   └── bilibili_config.py  # B站特定配置（API地址、请求头等）
├── core/                   # 核心模块
│   ├── __init__.py
│   └── browser.py          # Playwright浏览器管理
├── login/                  # 登录模块
│   ├── __init__.py
│   └── auth.py             # B站登录认证（扫码/Cookie）
├── client/                 # API客户端模块
│   ├── __init__.py
│   └── bilibili_client.py  # B站API客户端（搜索、详情）
├── crawler/                # 爬虫模块
│   ├── __init__.py
│   └── spider.py           # B站爬虫核心逻辑
├── store/                  # 存储模块
│   ├── __init__.py
│   └── backend.py          # 存储后端（JSON/CSV）
├── models/                 # 数据模型
│   ├── __init__.py
│   └── bilibili.py         # B站视频数据模型
├── tools/                  # 工具模块
│   ├── __init__.py
│   └── sign.py             # WBI签名工具
├── analysis/               # 分析模块
...
本章介绍的工程化规范在第11章「进阶综合实战项目」中得到了完整应用：
...
graph LR
    ...
    subgraph 第11章应用
        main_log[main.py 中的 setup_logger]
        settings[config/settings.py]
        bilibili_exc[异常处理与重试]
        project[完整项目目录]
    end
...
| 本章知识点 | 第11章应用位置 | 具体内容 |
|-----------|---------------|---------|
| loguru日志 | `main.py:setup_logger()` | 控制台+文件日志配置 |
| pydantic-settings | `config/settings.py` | BilibiliSettings配置类 |
| 自定义异常 | `client/bilibili_client.py` | B站API错误码处理 |
| tenacity重试 | `client/bilibili_client.py` | HTTP请求重试逻辑 |
| 项目结构 | 整个项目目录 | 11个模块的分层设计 |

> **学习建议**：在学习本章后，可以先预览第11章的项目结构，了解这些工程化规范是如何在实际B站爬虫项目中应用的。
